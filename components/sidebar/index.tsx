  ...
  return (
    <nav className="fixed flex h-screen basis-[10%] flex-col items-center justify-between gap-12 bg-xeat-dark-blue p-5 pr-0 text-white">
      <div className="flex w-full flex-col items-center gap-12">
        <Link href={"/"} className=" pr-5 md:w-full">
        ...
        <div className="flex w-full flex-col gap-5">
          ...
          <Link
            href={"/marketplace"}
            className={`flex justify-center rounded-l-xl py-4 pr-5 ${
              path?.includes("/marketplace") && "bg-xeat-black"
            }`}
          >
            <GiftIcon className={`h-7 w-7 ${path?.includes("/marketplace") ? "stroke-white" : "stroke-xeat-grey"}`} />
          </Link>
          <Link
            href={"/mycollections"}
            className={`flex justify-center rounded-l-xl py-4 pr-5 ${
              path?.includes("/mycollections") && "bg-xeat-black"
            }`}
          >
            <StarIcon className={`h-7 w-7 ${path?.includes("/mycollections") ? "stroke-white" : "stroke-xeat-grey"}`} />
          </Link>
          <Link
            href={"/events"}
            className={`flex justify-center rounded-l-xl py-4 pr-5 ${path?.includes("/events") && "bg-xeat-black"}`}
          >
            <UserIcon className={`h-7 w-7 ${path?.includes("/events") ? "stroke-white" : "stroke-xeat-grey"}`} />
          </Link>
        </div>
      </div>
